...
New feature (anything bigger than a bug fix)? Before designing: find or create the spec — check `<component>/docs/specs/`; if none matches, classify the component against `docs/templates/spec-archetypes.md` (six archetypes) and start from the matching blank in `docs/templates/` (interview: `prompts/spec-bootstrap.md`). Gate with `bash scripts/spec-health.sh`.
