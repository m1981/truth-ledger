...
| **Every push, or by hand** | `truth ttl-scan` — all that remains of `invalidate-scan` after its path and anchor arms were retired, and the system's only clock reader. TTL expiry (ADR-019) is the one thing reproduction provably cannot detect: a claim whose TTL runs out today reproduces perfectly today | **git or operator** | ✅ Fully |
...
2. On every push to main, run `truth reproduce` (blocking: exit 7 = a live capsule stopped reproducing, exit 8 = it examined nothing) and then `truth ttl-scan`, **auto-committing any resulting expiry records back** with a bot identity (`TRUTH_ACTOR=ci-scanner`). That closes the loop with zero humans for the half a machine can decide — the clock — while the half that needs judgment blocks the pipeline instead of filing a guess. The shipped `truth-scan.yml` does exactly this.
...
flowchart TB
    ...
    subgraph cli["ledger entry points (scripts/truth)"]
        ...
        SCAN["truth reproduce (read) +<br/>truth ttl-scan (the clock)"]
...
flowchart TB
    ...
    subgraph scan["truth-scan.yml"]
        ...
        S1["reproduce (blocking) then<br/>ttl-scan, TRUTH_ACTOR=ci-scanner"]
