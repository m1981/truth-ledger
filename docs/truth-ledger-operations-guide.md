...
**The advisory block at filing** (v0.9.20/ADR-034): every post-append filing notice — the v0.9.11 hollow-VERIFIED exit warning (`evidence command exited N ...`, non-blocking by design: a non-zero-but-stable probe can be a legitimate fact), the v0.9.14/ADR-032 scope-ok default-expiry notice (a `--scope-ok` filed without `--ttl-days` is stamped a default 30-day expiry, decays per ADR-019, and `reaffirm` arm 1 routes it to re-file, re-firing the ADR-007 gate), and the FS-1 half-life note (now post-append) — renders as **one contiguous stderr block**, each line prefixed `truth: advisory:`. Grep for that prefix; never `tail -1` a capture and assume you saw the advice (the QB-011 lesson). Under `--json` the echoed record carries the same messages in an `advisories` array, so harnesses parsing stdout cannot lose them. A clean filing prints no advisory line; the commit-gate banner stays outside the block by design (it must fire even on refused filings). Canary FAULT GS pins the block, the `--json` mirror, and the staged gate order behind it.
