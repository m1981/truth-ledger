...
> Reader: any developer operating a truth ledger day-to-day | Enables: knowing every point where the ledger executes, spotting it firing, and automating everything except the three judgments that must stay human | Update-trigger: CLI trigger surface or hook wiring changes (current: CLI v0.10.0 — content re-synced at v0.9.13 on 2026-07-20, ADR-032/033 override-decay content added 2026-07-21; header pinned in lockstep by TestCrossSurfaceVersions since v0.9.13. Surface changes since the v0.6.2 diagrams below, in order: `done` executes a declared acceptance oracle (ADR-014, v0.7.0); `impact --inverse` is the backward-trace audit row (issue #5, v0.7.1); `baseline` the release-accounting row (issue #3, v0.8.0); `contradicts` the consistency row (issue #4, v0.9.0); every write verb carries the commit-gate banner and VERIFIED filing the exit-code warning (v0.9.11); `reaffirm` is the mechanical re-confirmation row (ADR-030, v0.9.12); v0.6.3 added output within doctor, v0.6.4 a flag within premise; v0.9.20/ADR-034 folds post-append filing notices into one `truth: advisory:` stderr block, mirrored into `--json` output — the signatures in §2 changed shape, the semantics did not; v0.9.21/ADR-035 adds the positive-claim exit gate — a VERIFIED filing whose sentence carries no negation token and whose evidence exits non-zero is now REFUSED at intake naming ADR-035, `--evidence-exit-ok "<basis>"` is the stored, counted override, and absence proofs keep the advisory path; v0.9.22/ADR-036 adds the tombstone citation gate -- `verdict retracted` and `done --cancel` refuse with exit 6 while scope-covered files (`.truth/citation-scope`, default `docs/specs/**`) cite the id, and the read-only `truth citations <id>...` preflight sweeps before a batch; v0.9.23/ADR-037 adds the recipe lints -- `-n`, version- and date-shaped literals warn in the advisory block with path/schema-id/frozen-date carve-outs -- and the `.truth/generated-paths` refusal for watches on generated artifacts, `--generated-ok` decaying like `--scope-ok`; v0.9.24/ADR-038 adds the dirty-watch advisory -- filing a claim whose watched path carries uncommitted changes, an untracked file under a glob watch, an uncommitted rename, or a merge-conflict state voices a restale-at-birth line in the block, never a refusal; v0.9.25/ADR-039 adds the blast forecast -- a path filing stamps `blast_forecast` (distinct commits touching the watch in 30d, an upper bound on stalings) and voices an advisory at or above a self-calibrating floor, with shallow/unborn history degrading loudly; `truth stats` gains the `blast` churn section: observed-vs-forecast, per-path staler ranking, effective floor; v0.9.26/ADR-040 changes no trigger: the shipped evidence allowlist loses rg/file/date after a per-program audit found exec/write channels on them, and doctor's grey-zone advisory gains the same three so an existing consumer -- whose allowlist the template never clobbers -- is warned rather than silently trusted; v0.9.27/ADR-043 adds one read verb and one flag: `truth vocab [--json]` exports the machine vocabulary (statuses, active set, verdict->status map, ADR-001 premise derivations, and the satellites' citation-blocking set) -- spec-health/fact-health now fetch their blocking set from it at runtime and fail loudly when the call fails; `done` gains `--json` (one object {issue, event, claim, accept, advisories}, the SI-3 surface extended to claim-at-death) and now refuses a missing --basis or invalid transition BEFORE the cancellation ceremony/citation sweep, matching `verdict`'s order -- refusal strings unchanged; v0.9.28/ADR-044 changes no trigger and no verb: the single-file CLI is split into the `truthlib/` package (registry/kernel/evidence/policy/gates/advisory/shellio/cli) behind the same thin `scripts/truth` entry -- every invocation, refusal, exit code and advisory byte-identical, proven by the unchanged 243-arm canary; operators notice only that a deployment now copies `truthlib/` beside `scripts/`; v0.9.29/ADR-045 adds one hook and one lock, no verb change: `install-hooks.sh` now writes a third hook, `pre-merge-commit` (same `check-truth` body — git runs it, never pre-commit, when a merge auto-commits, the union-merge sync path), `doctor` WARNs when pre-commit is wired locally without it (CI-gated repos exempt), and every write verb serializes on an exclusive flock of `<git-dir>/truth-ledger.lock` around its whole load→gates→append section — a second same-machine writer now waits (typically milliseconds) instead of racing the intake gates; a crashed holder's lock dies with its process, nothing to clean up; v0.9.30/ADR-046 REMOVES trigger surface (the one deliberate behavior change of the migration): `--concern` is gone from `claim` and `list`, `truth stats` loses its separation/overrides/blast/concerns sections in plain and --json (it keeps status/tier counts, verdict rates, half-life, queue aging), doctor loses the verifier-separation check, and intake stops stamping `blast_forecast` (the advisory still voices from a live computation) — the moved reports are meta-repo Tier C instruments: instruments/separation-report.py, instruments/override-velocity.py, instruments/blast-report.py, instruments/concern-tag.py, each with --json, gated by scripts/test-instruments.sh; legacy records carrying concerns/blast_forecast keep validating (admission rule, ADR-046); v0.9.31/ADR-047 is docs-only and changes no trigger: every Tier B gate now carries a named adoption metric, instrument source, and next-review date in the meta-repo gate-metric registry (docs/governance/gate-metrics.md), read as the monthly hand-audit's first item (§4); v0.9.32 changes no trigger and no ceremony -- two intake refusals on argument SHAPE: `truth citations` refuses an arg that is not a ledger id (tr-hex8/wk-hex8) and sweeps nothing, and `truth premise` refuses a malformed claim id or --supersedes ref before it appends, so a normal verb can no longer write a record `truth validate` and the commit gate then reject; v0.10.0 REMOVES two trigger rows and adds one, the first trigger-surface shrink since v0.9.30: `invalidate-scan` and `reaffirm` are gone (Reproduce-on-Read steps 2.5/2.6 -- the path/anchor proxy fired 1997 times for 71 judged divergences, and `truth reproduce` answers the same question semantically at read time), `truth health` is the new composed-projection read verb, and TTL expiry stops being a trigger at all: ADR-057 derives it inside `fold(events, now_dt=...)`, so no verb, record, scan or bot commit materializes the clock. `ttl-scan`, the interim verb step 2.6 narrowed `invalidate-scan` into, never shipped in a tagged release)
...
| **Every commit touching the ledger** | `check-truth.sh` via pre-commit hook — and, since v0.9.29/ADR-045, via `pre-merge-commit` for merge auto-commits (INV-A prefix check + schema validation) | **git, automatically** | ✅ Fully |
| **Every merge/pull** | nothing, deliberately — the post-merge hook is inert since the Reproduce-on-Read refactor (step 2.4). It used to run `invalidate-scan`, a SYNTACTIC proxy (did git touch a watched path) measured at 3.6% positive predictive value on this ledger: 1997 records against 71 judged divergences | **git, automatically** | ✅ Fully — as a deliberate no-op |
| **Every push** | `truth reproduce` via pre-push hook (written by `install-hooks.sh` since step 2.6): re-runs every LIVE claim's recorded evidence capsule and asks the SEMANTIC question — does it still produce what it produced? ~8ms per capsule. Blocks on exit 7 (a capsule stopped reproducing — a human judges it) and on exit 8 (the sweep examined zero claims; measuring nothing is a failure, ADR-042 rule 2). Files NOTHING on success: on-read verification stores no state | **git, automatically** | ✅ Fully |
| **Every read** | TTL expiry, derived inside `fold(events, now_dt=…)` (ADR-057). It has no verb and no trigger of its own: `truth list --stale`, `truth queue` and `truth health` show it because they fold with a clock. TTL expiry (ADR-019) is the one thing reproduction provably cannot detect — a claim whose TTL runs out today reproduces perfectly today — and it is also the one thing that needs no scan, because it is arithmetic over the claim's own `ts`. Until ADR-057 this row read `truth ttl-scan` and cost a scan, a bot commit and a push to say it | **nobody — it is a projection** | ✅ Fully |
| **Every push (meta-repo only)** | the release battery via pre-push hook (`scripts/release-battery.sh`, 2026-08-01): version lockstep, fact-health, doc-health, the core and v04 suites always; the 45s canary only when the pushed range touched the CLI or the suite (an unresolvable range widens to ALL — unknown scope must mean MORE checking). Since v0.9.34/ADR-048 it also runs the four formerly-orphaned gate suites (fact-health, session-digest, instruments, whisper — each judged by its own "N caught, M missed" line) and the `gate-reachability` sweep, which FAILS the push when any tracked check is invoked by no scheduled root; the battery's own mutation gate rides it too, scoped to pushes that touch the battery. 7 arms before v0.9.34, 13 after. Every arm reports WHAT IT EXAMINED, and an arm that examined nothing FAILS rather than passing. Before this the content-judging checks were manual and the core suite was failing at HEAD unnoticed. Preceded by the release tag-check: WARN when the CLI's stated version has no git tag (copier consumers resolve versions from tags, so an untagged release is invisible to them), FAIL when the tag's tree states a different version (mispointed tag) | **git, automatically** | ✅ Fully — untemplated like the whisper: releasing the template is meta-repo policy, not consumer machinery |
...
| Mechanical re-confirmation | `truth reproduce [--json]` — re-runs every LIVE claim's recorded capsule and classifies what came back; a READ verb that files nothing in either direction | Human, script, or the pre-push hook | ✅ The *mechanical* half only: it NAMES the claims whose capsule stopped producing and decides none of them (see §3, rung 3) |
| Triage | `truth queue` | Human, daily | ✅ The *surfacing*; not the deciding |
| Efficacy metrics | `truth stats [--json] [--since ts]` (status/tier counts, verdict rates by subtype, per-tier claim half-life, queue aging — the mechanical half of the monthly audit, v0.6/FS-1). Since v0.9.30/ADR-046 the instrument sections live OUTSIDE stats as meta-repo Tier C scripts: `instruments/override-velocity.py` (the ADR-033 override counts + verbatim-repeat advisory), `instruments/separation-report.py` (ADR-010 separation evidence, formerly also a doctor check), `instruments/blast-report.py` (ADR-039 churn, forecasts computed on read), `instruments/concern-tag.py` (legacy 42010 tag tally — the field is closed to new records) | Human, monthly (or CI report) | ✅ The *measuring*; the hand-audit judgment stays human (§4) |
...
**Rung 1 — hooks that survive clones.** Local `.git/hooks` die on every clone, so shims-in-hooks protects one machine. Promote to the committed hooks dir the template already ships: `.githooks/` (pre-commit, pre-merge-commit, post-commit, post-merge — all four executable; pre-merge-commit delegates to pre-commit so the merge-auto-commit class carries the same gate, ADR-045), activated by one line per clone, `git config core.hooksPath .githooks`. Note the two wirings are exclusive: `install-hooks.sh` refuses to write `.git/hooks` shims when `core.hooksPath` is set, precisely so dead files never impersonate a gate. The committed hooks update through normal file diffs.
...
2. On every push to main, run `truth reproduce` (blocking: exit 7 = a live capsule stopped reproducing, exit 8 = it examined nothing), then **print** what the clock has expired into the run summary. The half a machine can decide — the clock — needs no commit at all since ADR-057: it is derived on every read, so the job only has to make it VISIBLE, and the shipped `truth-scan.yml` runs with `permissions: contents: read`. It used to run `truth ttl-scan` and auto-commit the expiry records back under a bot identity, which meant an L1 instrument holding write access to the branch it measures; the loop guard, the `[skip ci]` marker and the concurrency queue all existed to contain that one write and left with it.
...
**Rung 3 — automate the verification dispatch.** First cut the volume, then decide nothing by machine. Until v0.10 this rung was `truth reaffirm` (ADR-030), which auto-filed an `agree` whenever a stale claim's evidence output hash still matched. It is **retired**, and the reason is worth keeping: after the Reproduce-on-Read refactor the only route to `stale` is TTL expiry, which was reaffirm's first arm and an unconditional refusal — every input the verb could still receive was one it declined by contract.

`truth reproduce` replaces it and is **stricter**: it re-runs every LIVE claim's recorded capsule and sorts the result into `reproduces` / `capsule-stale` / `unexecutable` / `no-capsule`, then **files nothing in either direction**. A green sweep leaves no record, because on-read verification has no stored state to advance. A `capsule-stale` claim exits 7 and blocks the push until a human judges it — `verdict <id> diverge`, or an `agree --refresh-evidence "<why the sentence still holds>"` (ADR-051). That refusal to auto-agree is the point: whether a changed hash is mechanical drift or a moved fact is ADR-012's judgment, and a sweep has no judge.

Execution goes through the SAME screened path `verdict --recheck` uses — `screen_evidence_command` against the CURRENT allowlist, so committed policy now, not at filing time. The named residual is unchanged in shape: the capsule checks the command's *output*, which can be narrower than the watch, so **keep evidence commands as wide as their `evidence_paths`** — or, better since v0.10, keep the watch as narrow as the command actually reads (the FAZA 3 rule: watch exactly the paths the recipe opens).
...
Since v0.9.31 (ADR-047) the audit's **first read is the gate-metric registry**: [docs/governance/gate-metrics.md](governance/gate-metrics.md), the table of every Tier B gate and counted override with its adoption metric, Tier C instrument source, current value, and next-review date. Re-pull the instrument values, apply the three-question retirement test to any row that is due, and append minutes there. This adds no ceremony — it is the same audit slot, now opening on data instead of recollection.
...
flowchart TB
    ...
    subgraph cli["ledger entry points (scripts/truth)"]
        ...
        SCAN["truth reproduce (read);<br/>the clock is folded, not scanned"]
...
sequenceDiagram
    ...
    participant POC as pre-push hook<br/>(truth reproduce)
...
flowchart TB
    ...
    subgraph scan["truth-scan.yml (read-only since ADR-057)"]
        S1["reproduce (blocking)<br/>writes nothing"]
        S2["list --stale<br/>(fold applies the clock)"]
        S4["run summary:<br/>'facts expired by the clock'"]
        S1 --> S2 --> S4
    end
...
records up: that manufactures churn and a false air of intentionality.
  ...
  *One deliberate exception, at the set level.* The ADR **series**
  carried a single RECORD-integrity claim — the ADR-series count sentinel, retired 2026-08-16, watching
...
watcher:
  ...
  | `docs/truth-ledger-explained.md` — the explainer | `tr-f8b946cd` (the Scope header and the CLI agree on the version, checked in BOTH directions and with neither the sentence nor the recipe naming the version). Two narrowings are folded into that one line, and both were forced by evidence. First: an ancestor asserted the whole page was version-synced, and four verifiers falsified it via four different stamps — a glossary CLI stamp, the schema `$id`, the ADR counts, the glossary size. A two-string grep cannot carry a page-wide property, so the sentence was cut to what it proves (ADR-007, applied to this repo's own prose). Second: every generation after that still hardcoded the version on both sides, so each one died at the next release bump. Measured across this ledger, a recipe naming a version literal has a **96% death rate** (24 of 25 dead); this claim's own lineage burned three ids that way. The recipe now extracts the v-string from `template/scripts/truth` line 2 and checks the other surface for it, so the assertion is *they agree*, not *both say X* — and a release bump leaves it true |
  | `template/docs/beads-integration-guide.md` — the ready/adapter seam | `tr-301931eb` (`bd ready --json` default + adapter seam) |
  | `template/.truth/README.md` — the tracker-seam contract | `tr-4a666db0` (+ `tr-891efd02`, `tr-789b11be`, `tr-4df1a9fd`) |
