...
One more verb for the honest edge case (v0.6.4, ADR-013): when a premise dies *genuinely* — the fact was wrong, and its correction lives under a new claim id — re-verifying would be dishonest, and the work would otherwise stay HELD forever. `truth premise <issue> <new-id> --supersedes <old-id>` files an auditable redirect: refused while the old premise is live or unverified (the states that need no rescue), and the replacement is judged by the same matrix afterward. It re-targets protection; it never removes it. One exception makes the redirect honor the tombstone rule above (v0.9.3, ADR-017): superseding a *retracted* premise needs the same human gate as the retraction itself (`TRUTH_HUMAN` + the typed-id ack). Retraction is a human's terminal veto, and releasing the work it blocks is the same class of act as imposing it — so an agent can't spend it via this verb. The *mechanical* dead states (stale, diverged, cannot_verify, missing) stay ungated: nobody decided those, so the low-friction redirect is right for them.
...
So premises gate the *start* of work and acceptance oracles gate the *end* — and there's now a pattern for watching the *middle*: spec-coverage manifests. A spec's testable assertions each get a name (an inline `SC-<slug>-NNN` id), the tests that check them cite that name in their docstrings, and two ordinary sentinel claims — one diffing spec against manifest, one diffing test citations against manifest — go stale on edits to the paths they watch and ask for a look whenever the re-run diff is non-empty. It's a review trigger, not a gate: a citation only proves a test *mentions* the assertion, and it's still the acceptance oracle that proves anything ran. The full design, tested recipes, and its honest limits live in `docs/growth-gate/spec-coverage-manifests.md`.
...
| `truth premise <issue> <new-id> --supersedes <old-id>` | Redirect a genuinely dead premise to its corrected claim (ADR-013; refused while the old one is live or unverified; a *retracted* premise needs the human gate — ADR-017) |
