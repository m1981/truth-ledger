...
| waivers | every gate that can be lifted, and by what | `docs/waivers.md` | live; membership gated, contents DECLARED | `instruments/waiver-index.py`, gated by `template/scripts/test-integrations.py` — BOTH directions against the CLI parser: a listed flag the parser does not accept, and a `--*-ok` flag the parser accepts with no row. The `admitted on` and `verbs` columns are checked against the parser too; `stamp`, `decays` and `governing record` are NOT, and the file says so. No baseline: there is no backlog to freeze, because the reverse direction is TOTAL -- every flag the parser accepts must be a waiver row or an entry in `.truth/waiver-not-an-override` with a finished reason. The four CONTENT columns (gate lifted, stamp, decays, governing record) are held by reading and have already produced two defects; item 0b is DECLARED, not DONE |
