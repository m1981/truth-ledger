...
| INV-C | VERIFIED claims still require watched `--paths` (or a TTL for facts outside the repo), but a watched-path change **no longer demotes**: reproduction-on-read decides (the INV-C successor, gated pre-push by `reproduce`). The path invalidator was retired at step 2.5 — measured PPV 3.6% (1997 firings, 71 judged divergences) | One claim staled by a mere path touch; or one VERIFIED claim accepted with neither paths nor TTL | Seeded `FAULT B` (step 2.5, **inverted**); the filing half by intake tests |
| INV-D | Recheck detects non-reproducing evidence | One hash mismatch scored agree | Seeded `FAULT C` (T1) — `recheck` must diverge when reality no longer matches |
...
| INV-F | An erased anchor commit does **not** invalidate a reproducible claim: the history moved, the fact did not, and `reproduce` answers the second question without guessing from the first | One claim staled by a history rewrite; or an `anchor unreachable` invalidation record written | Seeded `FAULT E` (step 2.6, **inverted**, plus the positive half: the capsule still reproduces) |
...
| INV-J | An ADR-051 refresh leaves a re-verified claim live **and** its recorded capsule reproducible. The advancing effective anchor this row used to name no longer defends anything: with the path invalidator retired there is no scan to undo the re-verification. TTL'd claims remain exempt (ADR-019): re-filed, not re-verified | One live claim whose recorded capsule cannot be produced | Seeded `FAULT L` (ADR-051, re-aimed at step 2.5) |
...
| INV-U | `truth stats`'s `overrides` section counts scope-ok filings, ADR-032 decay expiries, overridden duplicates, and unscreened filings exactly, and flags as a repeat only a `scope_basis` claim whose `tokens()` set equals an earlier claim's that is now dead (stale/diverged/retracted) — a live/unverified prior is not flagged (ADR-033) | A verbatim re-justification after a decay expiry not flagged; or a genuinely narrowed re-file (token set differs in substance) flagged | Tier C: `TestTierCInstruments.test_override_velocity_real_and_sandbox` (template/scripts/test-integrations.py). The canary `FAULT OV` arms this row used to name were retired by ADR-046 when the section left the template CLI, moved to `scripts/test-instruments.sh`, and moved again when 32022c6 replaced that scaffolding with the stdlib runner |
